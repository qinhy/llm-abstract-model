...
store.clean()
req = store.add_new_celery_request(
    url='http://localhost:8000/terminals/add', method='POST')
...
myprint(
    'req(params={"broker": "brokerX", "path": "/path/to/termial"},debug=debug,debug_data=dict())')
# ->  {'task_id': 'some-task-id', 'status': 'STARTED', 'result': {'message': 'Task is started'}}

req = store.add_new_celery_request(
    url='http://localhost:8000/terminals/', method='GET')
...
# get account info
account_info = store.add_new_celery_request(
    url='http://localhost:8000/accounts/info/', method='GET')
...
# get rates
get_rates = store.add_new_celery_request(
    url='http://localhost:8000/rates/', method='GET')
...
book_maker = [get_rates, decode_rates, llm, extract_json, to_book_plan]
book_maker = compose(*book_maker)
book = book_maker(json=acc,params=dict(symbol='USDJPY', timeframe='H4', count=30))

# Make book plan workflow
book_plan_workflow = store.add_new_workflow(
    # function id : [ arg param dependencies ... ]
    tasks={
        get_rates.get_id():      ['init_deps', 'rate_param'],
